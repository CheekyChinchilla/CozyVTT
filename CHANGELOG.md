...
- **Backups made from the Admin Dashboard can be restored again.** Restoring one failed with "Database restore failed", and the log said nothing useful. The database tools inside the backend image had drifted to a newer PostgreSQL than the database itself, and a backup written by the newer tools opens with a setting the database does not recognise, so the restore stopped on its fourth line. Every dashboard backup made on 1.4.0 is affected, and none of them is damaged: a restore that fails changes nothing. The image now carries tools that match the database, a restore skips that setting so the backups you already have load, and the reason for any failure is written to the backend log. Restoring also now replaces the whole database with the backup and then runs this version's migrations, so a backup from an older CozyVTT restores into a newer one, including a freshly installed one, and comes up to date on its own; before, a table added since the backup was made stopped the restore. A backup also restores onto an instance whose database user has a different name, which is what moving to a new machine with a fresh `.env` produces: it used to fail with "role does not exist", and now everything in it ends up owned by the instance's own database user. New backups, from the dashboard and from the command-line script, no longer record an owner at all. On an install without Docker the database role has to own the database for this; the deployment guide says how.

- **A PDF that will not show in the reader now says what to do about it.** Some browsers display nothing for a PDF opened inside a page, and the frame is blank white, so there is no telling that from a slow load. Safari is the one reported. A line above the document now offers to open it in a new tab, which works everywhere.

- **Previewing a token shows what that character remembers.** Picking a single token in Preview Player View drew only what it can see at that moment, with none of the ground its player had already explored, which made a projected screen look emptier than the player's own. The preview now shows the memory of whoever controls the token. **All player tokens** still shows none, since several players' memories laid over each other would describe nobody.

- **A token moved to another map arrives as itself.** Bringing tokens along when you switch maps, or moving one from the Token Manager or the right-click menu, rebuilt each token from a partial copy. A player's character came back listed as an NPC, and darkvision, hit points, disposition, the creature's stat block, its art mode, its rotation and its notes were all dropped on the way. The whole token now travels, and duplicating one keeps everything too, apart from the fresh hit points and cleared conditions a copy is meant to start with. A token too big for the map it arrives on is placed fully inside it instead of hanging off the edge.

- **Any token can be hidden from the map, not just objects.** Right-clicking a token offered **Hide from Players** only when it was an object, so a DM could not hide a monster from the map itself, though the Token Roster's eye and **Edit Token** always could. The right-click menu now offers it for every token, and a creature or object can be placed already hidden from the Token Manager, so a monster can be staged in a room before the table knows it is there.

- **Revealed fog no longer looks lost after a refresh.** On some instances every load of a fogged map left the DM with no fog state: no tint on their own map, a Player Preview with everything covered, and reveal boxes that could not be dragged, though nothing had been lost on the server. The page asked the server for the fog the moment the map loaded, and when the map arrived before the live connection had finished signing in, the request was sent ahead of the sign-in and dropped without a word. The request now waits for the connection to be ready, is sent again after a reconnect, and switching fog on for a map delivers its fog to everyone at once instead of relying on each screen to ask. Explored memory had the same gap and gets the same fix.

- **A player's token is the one they control, on screen as on the server.** The map also counted a token bound to one of the player's characters as theirs, while the server counts only the controller. When the two differed, for example after the DM handed a character's token to someone else or an integration created it without naming a controller, the player could pick the token up only to have the server put it back, and on a lit map they were shown ground the server sent them nothing for. The map now agrees with the server. A token created for a character with no controller named is now controlled by the character's owner, so that state no longer arises by accident; the DM can still hand control to anyone in Edit Token.

- **Preview Player View no longer gives away hidden or unseen tokens.** A token hidden from players still lent its sight to the preview, so a projected table saw the ground around a token it could not see; and pointing at any token, hidden, unlit or under fog, still opened its details panel with its name and position. A preview now looks only through tokens the players would have, and the pointer finds only what the preview draws. The picker no longer offers hidden tokens.

- **Preview Player View no longer shows where the lights are.** The markers the DM uses to select and move lights stayed on the map in the preview, so a table watching one projected screen could see every light's position, lit or not. They are hidden in the preview and return with the DM's own view; to move or edit a light, leave the preview first.
...
- **The bundled web server now forwards `/health` to the backend.** An uptime check against the site's own address was answered by the web page, which returns 200 for any path it does not know, so a dead backend looked healthy. `/health` now reaches the backend and answers 503 when the database is unreachable, as documented. Upgrading now recreates the bundled web server so a change to its configuration takes effect; until now `docker compose up -d --build` left a running web server on the configuration it started with. The route reference's session pause and end responses now describe what those routes return.
