...
- **Hit dice can be spent from the sheet and the roll menu.** ([#43](https://github.com/CheekyChinchilla/CozyVTT/issues/43)) A D&D 5e sheet has always had somewhere to record hit dice, and the field even suggests a dice formula — but the number was not clickable the way an ability score or an attack is, and it never appeared in the right-click **Roll...** menu. The only way to use one was to roll it by hand somewhere else and edit the count down yourself. Clicking your hit dice on the **Combat** tab now spends one: it rolls a single die plus your Constitution modifier, the result goes to the roll history like any other roll, and the count drops by one. The same entry appears under **Hit Dice** in the roll menu, so it works from a token as well as from the sheet. **It rolls one die, not the pool** — `5d10` means five d10 hit dice, and rolling the stored text would have rolled all five at once, which is not what spending one means. The rules let you decide whether to spend another after seeing each result, so click again for a second. **The hit points are not added for you**: CozyVTT rolls the die and keeps count of what is left, and you apply the healing with the **+** on the roster card, which is also where a negative Constitution modifier is handled — the rules floor what you regain at zero rather than taking hit points away. Once a pool is empty it stops being clickable, and a sheet whose hit dice were left blank or typed as something other than a die offers nothing rather than a roll it cannot make. Your DM can spend one on your behalf if you are away from the table, and it comes off your own sheet
